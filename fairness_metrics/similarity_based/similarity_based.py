...
class LipschitzFairness:
    def __init__(self, dataset, parameters):
        ...
        self.feature_columns = [name for name, is_protected in zip(dataset.i2c, protected_values) if is_protected]
        ...
        for i, j in itertools.combinations(self.indices.tolist(), 2):
            ...
            if prediction_diff > d:
                self.violations.append(
                    {"pair": (i, j), "prediction_diff": prediction_diff, "distance": d, "amount": prediction_diff - d}
                )
