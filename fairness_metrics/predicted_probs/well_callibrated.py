...
class well_calibration:
    ...
    def show(self, raw_results=False) -> go.Figure:
        ...
        for feature, data in self.results:
            ...
            if len(data) > 5:
                bin_ranges = (np.linspace(0, len(data) - 1, 4) + 0.5).astype(int)
                for i, ni in zip(bin_ranges[:-1], bin_ranges[1:]):
                    new_data = sum(data[i:ni, 1:]) / len(data[i:ni, 1:])
                    fig.add_trace(go.Line(name=f"from {data[i,0]} to {data[ni,0]}", x=bins, y=new_data))
            else:
                for group_data in data:
                    fig.add_trace(go.Line(name=f"{group_data[0]}", x=bins, y=group_data[1:]))
