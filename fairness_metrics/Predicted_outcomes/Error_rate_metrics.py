...
class Error_rate_metrics:
    def __init__(self, dataset, params):
        ...
        for attr in self.protected_attributes:
            ...
            self.results[attr] = {"fpr": fpr_dict, "fnr": fnr_dict}
    ...
    def show(self, raw_results=False) -> go.Figure:
        ...
        for attr_name in self.results:
            ...
            fig.add_trace(go.Bar(name="FPR", x=[str(k) for k in fpr.keys()], y=list(fpr.values())))

            fig.add_trace(go.Bar(name="FNR", x=[str(k) for k in fnr.keys()], y=list(fnr.values())))
